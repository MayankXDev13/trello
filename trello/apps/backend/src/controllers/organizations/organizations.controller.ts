...
const patchSchema = z.object({ name: z.string().min(1).max(100).optional(), description: z.string().max(500).optional().nullable() });
...
export const create = async (req: AuthRequest, res: Response) => {
  ...
  const [org] = await db.insert(organizations).values({ name: parsed.data.name, description: parsed.data.description }).returning();
  if (!org) return res.status(500).json({ message: "Failed to create organization" });
  await db.insert(userOrganizations).values({ userId: req.user!.userId, organizationId: org.id, role: "admin" });
  return res.status(201).json({ organization: org });
};

// GET /organizations - list my orgs
export const list = async (req: AuthRequest, res: Response) => {
  const rows = await db.select({ org: organizations, role: userOrganizations.role })
  ...
  return res.status(200).json({ organizations: rows.map((r: any) => ({ ...r.org, role: r.role })) });
};
...
export const patch = async (req: AuthRequest, res: Response) => {
  const { organizationId } = req.params as any;
  if (!z.string().uuid().safeParse(organizationId).success) return res.status(400).json({ message: "Invalid organizationId" });
  if (!(await isOrgAdmin(req.user!.userId, organizationId))) return res.status(403).json({ message: "Forbidden - admin only" });
  ...
  const [updated] = await db.update(organizations).set(updates as any).where(eq(organizations.id, organizationId)).returning();
...
export const remove = async (req: AuthRequest, res: Response) => {
  ...
  if (!z.string().uuid().safeParse(organizationId).success) return res.status(400).json({ message: "Invalid organizationId" });
  if (!(await isOrgAdmin(req.user!.userId, organizationId))) return res.status(403).json({ message: "Forbidden - admin only" });
  const [deleted] = await db.delete(organizations).where(eq(organizations.id, organizationId)).returning();
...
export const listMembers = async (req: AuthRequest, res: Response) => {
  ...
  const rows = await db.select({ membership: userOrganizations, user: users })
    .from(userOrganizations)
    .innerJoin(users, eq(users.id, userOrganizations.userId))
    .where(eq(userOrganizations.organizationId, organizationId));
  return res.status(200).json({ members: rows.map((r: any) => ({ id: r.user.id, username: r.user.username, email: r.user.email, role: r.membership.role, membershipId: r.membership.id })) });
};

const addMemberSchema = z.object({ userId: z.string().uuid(), role: z.enum(["admin", "member"]).default("member") });
export const addMember = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const [row] = await db.insert(userOrganizations).values({ userId: parsed.data.userId, organizationId, role: parsed.data.role }).returning();
    return res.status(201).json({ member: row });
  } catch (e: any) {
    if ((e as any)?.code === "23505" || (e as any)?.cause?.code === "23505" || String((e as any)?.cause?.message||"").includes("duplicate") || String(e?.message).includes("unique")) return res.status(409).json({ message: "Already a member" });
    throw e;
  }
};
...
export const patchMember = async (req: AuthRequest, res: Response) => {
  ...
  if (!z.string().uuid().safeParse(organizationId).success || !z.string().uuid().safeParse(userId).success) return res.status(400).json({ message: "Invalid id" });
  if (!(await isOrgAdmin(req.user!.userId, organizationId))) return res.status(403).json({ message: "Forbidden - admin only" });
  const parsed = patchMemberSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: "Validation error", issues: parsed.error.issues });
  const [updated] = await db.update(userOrganizations).set({ role: parsed.data.role })
    .where(and(eq(userOrganizations.organizationId, organizationId), eq(userOrganizations.userId, userId))).returning();
...
export const removeMember = async (req: AuthRequest, res: Response) => {
  ...
  if (!z.string().uuid().safeParse(organizationId).success || !z.string().uuid().safeParse(userId).success) return res.status(400).json({ message: "Invalid id" });
  if (!(await isOrgAdmin(req.user!.userId, organizationId))) return res.status(403).json({ message: "Forbidden - admin only" });
  const [deleted] = await db.delete(userOrganizations).where(and(eq(userOrganizations.organizationId, organizationId), eq(userOrganizations.userId, userId))).returning();
