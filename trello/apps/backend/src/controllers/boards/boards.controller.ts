...
export const patch = async (req: AuthRequest, res: Response) => {
  ...
  const [board] = await db.select().from(boards).where(eq(boards.id, boardId));
  if (!board) return res.status(404).json({ message: "Board not found" });
  if (!(await isOrgMember(req.user!.userId, board.organizationId)))
    return res.status(403).json({ message: "Forbidden" });
...
export const getFull = async (req: AuthRequest, res: Response) => {
  ...
  if (sectionIds.length > 0) {
    ...
    iss = await db
      .select()
      .from(issues)
      .where(inArray(issues.sectionId, sectionIds));
  }
  ...
  if (issueIds.length > 0) {
    assigneeRows = await db
      .select()
      .from(issueUsers)
      .where(inArray(issueUsers.issueId, issueIds));
  ...
  if (assigneeRows.length > 0) {
    ...
    for (const u of urows)
      userMap[u.id] = { id: u.id, username: u.username, email: u.email };
  }
    ...
    const secIssues = iss
      ...
      .map((issRow) => {
        const assignees = assigneeRows
          .filter((a) => a.issueId === issRow.id)
          .map((a) => userMap[a.userId] ?? { id: a.userId });
...
export const createSection = async (req: AuthRequest, res: Response) => {
  ...
  const [section] = await db
    .insert(sections)
    .values({ title: parsed.data.title, boardId })
    .returning();
