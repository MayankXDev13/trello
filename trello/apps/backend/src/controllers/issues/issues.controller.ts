...
export const move = async (req: AuthRequest, res: Response) => {
  ...
  const [targetSection] = await db
    .select()
    .from(sections)
    .where(eq(sections.id, parsed.data.targetSectionId));
  if (!targetSection)
    return res.status(404).json({ message: "Target section not found" });
  // ensure target section is in same org (via board)
  const targetOrgId = await getOrganizationIdBySection(
    parsed.data.targetSectionId,
  );
  if (targetOrgId !== orgId)
    return res
      .status(400)
      .json({ message: "Target section must be in same organization" });
  const [updated] = await db
    .update(issues)
    .set({ sectionId: parsed.data.targetSectionId })
    .where(eq(issues.id, issueId))
    .returning();
  return res.status(200).json({ issue: updated });
};
...
export const listAssignees = async (req: AuthRequest, res: Response) => {
  ...
  if (!orgId || !(await isOrgMember(req.user!.userId, orgId)))
    return res.status(403).json({ message: "Forbidden" });
  const rows = await db
    .select({ assignment: issueUsers, user: users })
    .from(issueUsers)
    .innerJoin(users, eq(users.id, issueUsers.userId))
    .where(eq(issueUsers.issueId, issueId));
  return res
    .status(200)
    .json({
...
export const addAssignee = async (req: AuthRequest, res: Response) => {
  ...
  if (!(await isOrgMember(parsed.data.userId, orgId)))
    return res
      .status(403)
      .json({ message: "Assignee must be organization member" });
  ...
  try {
    const [row] = await db
      .insert(issueUsers)
      .values({ issueId, userId: parsed.data.userId })
      .returning();
...
export const removeAssignee = async (req: AuthRequest, res: Response) => {
  ...
  if (
    !z.string().uuid().safeParse(issueId).success ||
  ...
  const [deleted] = await db
    .delete(issueUsers)
    .where(and(eq(issueUsers.issueId, issueId), eq(issueUsers.userId, userId)))
    .returning();
  if (!deleted)
...
export const createComment = async (req: AuthRequest, res: Response) => {
  ...
  const [comment] = await db
    .insert(comments)
    .values({ content: parsed.data.content, issueId, userId: req.user!.userId })
    .returning();
