...
import { getOrganizationIdByBoard, getOrganizationIdBySection, isOrgMember } from "../../lib/org";

const patchSchema = z.object({ title: z.string().min(1).max(200) });

export const getOne = async (req: AuthRequest, res: Response) => {
  const { sectionId } = req.params as any;
  if (!z.string().uuid().safeParse(sectionId).success) return res.status(400).json({ message: "Invalid sectionId" });
...
export const patch = async (req: AuthRequest, res: Response) => {
  ...
  const [updated] = await db.update(sections).set({ title: parsed.data.title }).where(eq(sections.id, sectionId)).returning();
...
const issueCreateSchema = z.object({ title: z.string().min(1).max(200), description: z.string().max(2000).optional() });
...
export const createIssue = async (req: AuthRequest, res: Response) => {
  ...
  if (!parsed.success) return res.status(400).json({ message: "Validation error", issues: parsed.error.issues });
  const [issue] = await db.insert(issues).values({ title: parsed.data.title, description: parsed.data.description, sectionId }).returning();
...
export const listIssues = async (req: AuthRequest, res: Response) => {
  ...
  if (!orgId) return res.status(404).json({ message: "Section not found" });
  if (!(await isOrgMember(req.user!.userId, orgId))) return res.status(403).json({ message: "Forbidden" });
  const rows = await db.select().from(issues).where(eq(issues.sectionId, sectionId));
